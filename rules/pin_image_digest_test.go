...
func TestPinImageDigest_Dockerfile(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assertIssuesInDir(t, rules.PinImageDigest, linter.SeverityWarn, "devcontainer.json", linter.Devcontainer, src, dir, tt.want)
		})
	}
	...
	t.Run("an image named outright is reported at the property", func(t *testing.T) {
		...
		assertIssuesInDir(t, rules.PinImageDigest, linter.SeverityWarn, "devcontainer.json", linter.Devcontainer, `{"image": "ubuntu:24.04"}`, dir, want)
	})

	t.Run("a missing Dockerfile reports nothing", func(t *testing.T) {
		t.Parallel()
		assertIssuesInDir(t, rules.PinImageDigest, linter.SeverityWarn, "devcontainer.json", linter.Devcontainer, src, linter.Dir{FS: fstest.MapFS{}}, nil)
	})
	...
	t.Run("the Dockerfile a Compose service builds from is read", func(t *testing.T) {
		...
		assertIssuesInDir(t, rules.PinImageDigest, linter.SeverityWarn, "devcontainer.json", linter.Devcontainer, src, dir, want)
	})
	...
	t.Run("a document that is not an object reports nothing", func(t *testing.T) {
		...
		assertIssuesInDir(t, rules.PinImageDigest, linter.SeverityWarn, "devcontainer.json", linter.Devcontainer, `["Dockerfile"]`, dir, nil)
	})
}
