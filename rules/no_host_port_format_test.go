...
func TestNoHostPortFormat(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{"host:port entry", `{"forwardPorts": ["db:5432"]}`, []linter.Issue{
			{Path: "devcontainer.json", Line: 1, Col: 19, RuleID: "no-host-port-format",
		...
		{"mixed valid and host:port entries", `{"forwardPorts": [3000, "db:5432"]}`, []linter.Issue{
			{Path: "devcontainer.json", Line: 1, Col: 25, RuleID: "no-host-port-format",
				Message: `"forwardPorts" entry "db:5432" uses "host:port" format; Codespaces only supports a bare port number`},
		}},
		...
		{"host:port key", `{"portsAttributes": {"db:5432": {}}}`, []linter.Issue{
			{Path: "devcontainer.json", Line: 1, Col: 33, RuleID: "no-host-port-format",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			assertIssues(t, rules.NoHostPortFormat, linter.SeverityError, tt.src, tt.want)
		})
	}
}
