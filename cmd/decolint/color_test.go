	...
	for _, tt := range tests {
		t.Run(tt.want, func(t *testing.T) {
			...
			if got := tt.mode.String(); got != tt.want {
				t.Errorf("colorMode.String() = %q, want %q", got, tt.want)
			}
		})
	}
}
...
func TestUseColor(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "FORCE_COLOR=0 is not set", mode: colorAuto, tty: false, env: map[string]string{"FORCE_COLOR": "0"}, want: false},
...
func TestIsTerminal(t *testing.T) {
	...
	// A pipe stands in for the destinations decolint is normally redirected to: a real terminal
	// cannot be opened portably, so only the negative cases are covered here.
	r, w, err := os.Pipe()
	if err != nil {
		t.Fatal(err)
	}
